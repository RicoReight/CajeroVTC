...
let diaReset = loadDiaReset();       // día del mes para reset automático de propinas
...
function loadDiaReset(){
  const n = parseInt(safeStorage.get("uberCambioDiaReset"));
  return (n >= 1 && n <= 31) ? n : 20;
}
function saveDiaReset(){ safeStorage.set("uberCambioDiaReset", String(diaReset)); }

function loadUltimoResetPropinas(){
  return safeStorage.get("uberCambioUltimoResetPropinas") || null;
}
...
/* ---------- Mostrar precio en grande (ES/EN) ---------- */
